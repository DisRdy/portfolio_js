...
| `GET /project/{id}/download` | route closure | Worker download handler + R2 object stream |
...
- Eloquent -> prepared-statement repository functions in `src/data.ts`.
- Blade -> server-rendered HTML string views in `src/views.ts`.
- Local filesystem -> Cloudflare R2 binding `UPLOADS`.
